...
class RmitTbModel(basepop.BaseModel):
  ...
  def calculate_vars(self):
    ...
    self.vars[
      'lambda'] = self.params['mu'] * self.vars['population'] + self.params[
      'd'] * self.compartments['I']

    # infection
    self.vars[
      'beta_I'] = self.params['beta'] * self.compartments['I'] / self.vars[
      'population']
    for i_sigma in range(1, 4):
      self.vars[
        'sigma' + str(i_sigma) +
        '_beta_I'] = self.vars['beta_I'] * self.params['sigma' +
                                                       str(i_sigma)]
