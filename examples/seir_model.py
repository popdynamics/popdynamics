    ...
    def calculate_diagnostic_vars(self):
        ...
        for from_label, to_label, rate in self.fixed_transfer_rate_flows:
            ...
            if 'infectious' in to_label:
                self.vars[
                    'incidence'] += val / self.vars['population']
        ...
        for from_label, to_label, rate in self.var_transfer_rate_flows:
            ...
            if 'preinfectious' in to_label:
                self.vars[
                    'infections'] += val / self.vars['population']
...
class SeirDemographyModel(SeirModel):
    ...
    def __init__(self, param_dictionary):
        ...
        self.set_compartment(
            ...
            param_dictionary['start_infectious'])
        ...
        self.set_param('r0', param_dictionary['r0'])
        ...
        self.set_param(
            'infection_rate_progress',
            old_div(1.,
                    param_dictionary['duration_preinfectious']))
        self.set_param(
            'infection_rate_recover',
            old_div(1.,
                    param_dictionary['duration_infectious']))
        ...
        self.set_param(
            'demo_rate_death',
            old_div(1.,
                    param_dictionary['life_expectancy']))
        self.set_param(
            'demo_rate_birth',
            self.params['demo_rate_death'])  # closed population
    ...
    def calculate_vars(self):
        ...
        # set birth rate
        self.vars[
            'rate_birth'] = self.params['demo_rate_birth'] * self.vars['population']

        # calculate force of infection from beta (which was derived from R0 above)
        self.vars[
            'rate_force'] = self.params['infection_beta'] * self.compartments['infectious']
...
def plot_epidemiological_indicators(
        model,
        indicators,
        png,
        ylog=False):
...
def plot_compartment_proportions(model, png):
    ...
    for compartment in model.compartments:
        ...
        pylab.plot(
            ...
            label=compartment)
    r_n = [
        i * infection_params["flu"]["r0"]
        for i in compartment_props["susceptible"]
    ]
...
for infection in infection_params.keys():
    ...
    plot_epidemiological_indicators(
        ...
        ['incidence',
         ...
         'infections'],
        os.path.join(out_dir,
                     infection + "_indicators.png"))

    plot_compartment_sizes(
        model,
        os.path.join(out_dir,
                     infection + '_compartment_sizes.png'))
...
model.set_compartment(
    'immune',
    0.3 * model.init_compartments['susceptible'])
model.set_compartment(
    'susceptible',
    0.7 * model.init_compartments['susceptible'])
